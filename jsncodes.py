...
def load_preset():
    ...
    try:
        with open(memory_file, "r") as f:
            ...
            sound_objects = [pygame.mixer.Sound(file) for file in wav_files]
            for idx, sound in enumerate(sound_objects):
                sound.set_volume(channel_volumes[idx] * Seq.m_vol)
                channels[idx].set_volume((1 - pan_volumes[idx]) / 2, (1 + pan_volumes[idx]) / 2)
